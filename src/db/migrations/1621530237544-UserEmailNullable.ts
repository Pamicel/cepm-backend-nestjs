...
export class UserEmailNullable1621530237544 implements MigrationInterface {
  name = 'UserEmailNullable1621530237544';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE "temporary_user" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "email" varchar, "password" varchar, "permissionLevel" integer NOT NULL DEFAULT (1), "magicToken" varchar, "tokenIssued" date, "dateCreated" date NOT NULL, CONSTRAINT "UQ_e12875dfb3b1d92d7d7c5377e22" UNIQUE ("email"))`,
    );
    await queryRunner.query(
      `INSERT INTO "temporary_user"("id", "email", "password", "permissionLevel", "magicToken", "tokenIssued", "dateCreated") SELECT "id", "email", "password", "permissionLevel", "magicToken", "tokenIssued", "dateCreated" FROM "user"`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE "user" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "email" varchar NOT NULL, "password" varchar, "permissionLevel" integer NOT NULL DEFAULT (1), "magicToken" varchar, "tokenIssued" date, "dateCreated" date NOT NULL, CONSTRAINT "UQ_e12875dfb3b1d92d7d7c5377e22" UNIQUE ("email"))`,
    );
    await queryRunner.query(
